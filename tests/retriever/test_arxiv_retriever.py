...
def test_arxiv_retriever(config, monkeypatch):
    ...
    def mock_feedparser_parse(url):
        if (
            url
            == f"https://rss.arxiv.org/atom/{'+'.join(config.source.arxiv.category)}"
        ):
    ...
    parsed_results = [
        i for i in parsed_result.entries if i.get("arxiv_announce_type", "new") == "new"
    ]
...
def test_fetch_code_url_prefers_versionless_id_on_404(monkeypatch):
    ...
    def mock_get(url: str, timeout):
        ...
        if url.endswith("/1234.5678v1"):
            return MockResponse(404, {})
        if url.endswith("/1234.5678"):
            return MockResponse(200, {"githubRepo": "https://github.com/example/repo"})
        raise AssertionError(f"Unexpected url: {url}")

    monkeypatch.setattr(arxiv_retriever.requests, "get", mock_get)

    code_url = _fetch_code_url("1234.5678v1")

    assert code_url == "https://github.com/example/repo"
    assert requested_urls == [
        "https://huggingface.co/api/papers/1234.5678v1",
        "https://huggingface.co/api/papers/1234.5678",
    ]
