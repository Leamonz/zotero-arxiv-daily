...
def normalize_include_path_patterns(
    include_path: list[str] | ListConfig | None,
) -> list[str] | None:
    ...
    if any(not isinstance(pattern, str) for pattern in include_path):
        raise TypeError(
            "config.zotero.include_path must contain only glob pattern strings."
        )
...
class Executor:
    def __init__(self, config: DictConfig):
        self.config = config
        self.include_path_patterns = normalize_include_path_patterns(
    ...
    def fetch_zotero_corpus(self) -> list[CorpusPaper]:
        ...
        zot = zotero.Zotero(
            self.config.zotero.user_id, "user", self.config.zotero.api_key
        )
        ...
        def get_collection_path(col_key: str) -> str:
            if p := collections[col_key]["data"]["parentCollection"]:
                return (
                    get_collection_path(p) + "/" + collections[col_key]["data"]["name"]
                )
            else:
    ...
    def filter_corpus(self, corpus: list[CorpusPaper]) -> list[CorpusPaper]:
        ...
        new_corpus = []
        logger.info(
            f"Selecting zotero papers matching include_path: {self.include_path_patterns}"
        )
        for c in corpus:
            match_results = [
